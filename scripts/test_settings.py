...
class SettingsTest(unittest.TestCase):
    def test_updates_only_target_version_and_accepts_mixed_project_environments(self):
        ...
        def api(path, patch=None):
            ...
            if path.endswith("/version"):
                return [{"id": "old", "version_number": "0.2.0-alpha.1"},
                        {"id": "new", "version_number": "0.3.0-alpha.1"}]
            if path == "/version/new":
        ...
        self.assertEqual("processing", module.sync(api, "project", "0.3.0-alpha.1",
                         {"environment": "client_and_server", "submit_for_review": True}))
    ...
    def test_missing_or_ambiguous_version_stops_before_writes(self):
        for versions in ([], [{"id": "a", "version_number": "v"}, {"id": "b", "version_number": "v"}]):
            calls = []
            def api(path, patch=None):
                calls.append(patch)
                return versions
            with self.assertRaises(ValueError):
                module.sync(api, "p", "v", {"environment": "client_and_server"})
